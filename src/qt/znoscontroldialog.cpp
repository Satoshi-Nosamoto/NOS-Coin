...
#include "ui_znoscontroldialog.h"
...
std::list<std::string> ZNosControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNosControlDialog::listMints;

ZNosControlDialog::ZNosControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNosControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNosControlDialog::~ZNosControlDialog()
{
    delete ui;
}

void ZNosControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNosControlDialog::updateList()
{
...
void ZNosControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNosControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNos_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNosControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNosControlDialog::GetSelectedMints()
{
...
void ZNosControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
