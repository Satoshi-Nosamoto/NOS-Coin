                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNOS ought to be enough for anybody." - Bill Gates, 2017
    ui->zNOSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNOSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNOS->setEnabled(false);
        ui->pushButtonMintzNOS->setToolTip(tr("zNOS is currently disabled due to maintenance."));

        ui->pushButtonSpendzNOS->setEnabled(false);
        ui->pushButtonSpendzNOS->setToolTip(tr("zNOS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNOSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNOS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNOS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNOS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNOS in ") +
...
void PrivacyDialog::on_pushButtonSpendzNOS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNOS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNOS
        sendzNOS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNOS
    sendzNOS();
}

void PrivacyDialog::on_pushButtonZNosControl_clicked()
{
    ZNosControlDialog* zNosControl = new ZNosControlDialog(this);
    zNosControl->setModel(walletModel);
    zNosControl->exec();
}

void PrivacyDialog::setZNosControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNosSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNOS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Nos Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNOSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNOS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NOS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNOS</b>";
    ...
    // use mints from zNos selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNosControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNosControlDialog::GetSelectedMints();
    }
    ...
    // Clear znos selector in case it was used
    ZNosControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNos Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Nos, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNos Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNOS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNOS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNOS "));
    ui->labelzNOSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNOSSyncStatus->setVisible(fShow);
}
